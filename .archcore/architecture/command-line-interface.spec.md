...
5. WHEN передан `--timing-offset-ms`, CLI MUST передать значение генератору ASS.
...
2. IF timing offset выходит за диапазон, THEN CLI MUST завершиться с понятным сообщением.
